...
class BattleEntity:
    ...
    def reset_turn_state(self) -> None:
        ...
        self.reducir_cooldowns_habilidades()
    ...
    def _get_default_habilidades(self) -> Dict[str, Habilidad]:
        """Habilidades por defecto usando la nueva clase Habilidad"""
        # Puedes expandir según tu GDD y clases
        return {
            "ataque_basico": Habilidad(
                id="atk_basic",
                nombre="Ataque Básico",
                descripcion="Ataque básico sin costo de PH",
    ...
    def puede_usar_habilidad(self, nombre_habilidad: str) -> bool:
        """Verifica si puede usar una habilidad tipo Habilidad"""
        if nombre_habilidad not in self._habilidades:
            return False
        habilidad = self._habilidades[nombre_habilidad]
        return habilidad.puede_usar(self)

    def usar_habilidad(self, nombre_habilidad: str, objetivo=None, battle=None) -> str:
        """Usa una habilidad tipo Habilidad"""
        if nombre_habilidad not in self._habilidades:
            return f"Habilidad {nombre_habilidad} no existe"
        habilidad = self._habilidades[nombre_habilidad]
        resultado = habilidad.usar(self, objetivo, battle)
        return resultado

    def reducir_cooldowns_habilidades(self):
        """Reduce todos los TdE en 1 (llamar al final del turno)"""
        for habilidad in self._habilidades.values():
