...
class BattleEntity:
    ...
    def usar_habilidad(self, habilidad_id: str, objetivo=None, battle=None) -> str:
        ...
        for hab in self._habilidades:
            if hab.id == habilidad_id:
                resultado_original = hab.usar(self, objetivo, battle)
                break
        if resultado_original is None:
            return "Habilidad no encontrada"
        # Pasiva simplificada: AMP recupera PH
        if habilidad_id == "amp" and "✅" in resultado_original:
            ph_recuperado = 25
            self.stats.current_ph = min(self.stats.max_ph, self.stats.current_ph + ph_recuperado)
            return resultado_original + f" [+{ph_recuperado} PH]"
