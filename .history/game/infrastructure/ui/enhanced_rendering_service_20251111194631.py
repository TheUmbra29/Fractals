...
class EnhancedRenderingService(RenderingService):
    ...
    def render_battle(self, battle, game_context: GameContext, valid_moves: List[Position] = None) -> None:
        ...
        for entity in battle._entities.values():
            ...
            # Calcular la posición visual exacta de la celda lógica con camera_offset
            cell_rect = pygame.Rect(
                self.grid_offset[0] + entity.position.x * self.cell_size - self.camera_offset[0],
                self.grid_offset[1] + entity.position.y * self.cell_size - self.camera_offset[1],
                self.cell_size, self.cell_size
            )
            # Resaltar Ricchard en su celda lógica (con camera_offset)
        ...
        if (game_context.current_state == GameState.TRACING_ROUTE and 
            ...
            if selected_entity:
                ...
                # Marcadores visuales para depuración alineados a la celda lógica
                def cell_center(pos):
                    return (
                        self.grid_offset[0] + pos.x * self.cell_size + self.cell_size // 2,
                        self.grid_offset[1] + pos.y * self.cell_size + self.cell_size // 2
                    )
                start_center = cell_center(selected_entity.position)
                pygame.draw.circle(self.screen, (0,0,255), start_center, 12)
                if game_context.current_route.path:
                    end_center = cell_center(game_context.current_route.path[-1])
                    pygame.draw.circle(self.screen, (255,0,0), end_center, 12)
                    first_center = cell_center(game_context.current_route.path[0])
                ...
                # Dibujar todos los puntos de la ruta como círculos numerados alineados
                font = pygame.font.Font(None, 18)
                for idx, pos in enumerate(game_context.current_route.path):
                    pt_center = cell_center(pos)
    ...
    def _render_entity(self, entity, is_selected: bool = False, is_marked_for_dash: bool = False):
        ...
        # Calcular la posición visual exacta de la celda lógica con camera_offset
        cell_rect = pygame.Rect(
            self.grid_offset[0] + pos.x * self.cell_size - self.camera_offset[0],
            self.grid_offset[1] + pos.y * self.cell_size - self.camera_offset[1],
            self.cell_size, self.cell_size
        )
