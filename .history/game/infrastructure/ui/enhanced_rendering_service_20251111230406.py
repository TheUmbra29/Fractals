...
class EnhancedRenderingService(RenderingService):
    ...
    def __init__(self, screen_width: int = 800, screen_height: int = 600):
        ...
        self.camera_offset = (0, 0)  # Inicialización por defecto para desplazamiento de cámara
        from core.domain.services.cover_system import CoverType
        self.cover_colors = {
            CoverType.FULL: (100, 100, 200, 180),  # Azul semi-transparente
            CoverType.HALF: (200, 100, 100, 120)   # Rojo semi-transparente
        }
    ...
    def render_battle(self, battle, game_context: GameContext, valid_moves: List[Position] = None) -> None:
        ...
        # Renderizar estructuras de cobertura
        self._render_cover_structures(battle.cover_system)
        ...
        if (game_context.current_state == GameState.ABILITY_MENU and self.ability_menu is not None):
            self.ability_menu.draw(self.screen)
        # Renderizar información de habilidades en HUD
        self._render_habilidades_hud(battle)
    ...
    def _render_cover_structures(self, cover_system):
        """Renderiza las estructuras de cobertura"""
        for cover in cover_system.cover_structures:
            self._render_cover(cover)

    def _render_cover(self, cover):
        """Renderiza una estructura de cobertura individual"""
        from core.domain.entities.cover import CoverType, CoverStructure
        screen_x = self.grid_offset[0] + cover.position.x * self.cell_size - self.camera_offset[0]
        screen_y = self.grid_offset[1] + cover.position.y * self.cell_size - self.camera_offset[1]
        cover_surface = pygame.Surface((self.cell_size, self.cell_size), pygame.SRCALPHA)
        color = self.cover_colors[cover.cover_type]
        cover_surface.fill(color)
        if cover.cover_type == CoverType.FULL:
            pygame.draw.rect(cover_surface, (255, 255, 255, 100), 
                           (5, 5, self.cell_size-10, self.cell_size-10), 2)
        else:
            if cover.position.x % 2 == 0:
                pygame.draw.rect(cover_surface, (255, 255, 255, 100), 
                               (5, 5, self.cell_size//2-5, self.cell_size-10), 2)
            else:
                pygame.draw.rect(cover_surface, (255, 255, 255, 100),
                               (self.cell_size//2, 5, self.cell_size//2-5, self.cell_size-10), 2)
        self.screen.blit(cover_surface, (screen_x, screen_y))
        font = pygame.font.Font(None, 16)
        cover_text = "FULL" if cover.cover_type == CoverType.FULL else "HALF"
        text_surface = font.render(cover_text, True, (255, 255, 255))
        text_rect = text_surface.get_rect(center=(screen_x + self.cell_size//2, 
    ...
    def _render_state_info(self, game_context: GameContext):
        ...
        state_info = {
            ...
            GameState.ABILITY_MENU: "🧙 MENÚ DE HABILIDADES - Elige una habilidad"
        }
    ...
    def _render_habilidades_hud(self, battle):
        """Renderiza el HUD de habilidades"""
        player_entities = battle.get_player_entities()
        if not player_entities:
            return
        entity = player_entities[0]
        y_offset = 150
        hab_text = f"Habilidades de {entity.name}:"
        hab_surface = self.font.render(hab_text, True, (255, 255, 255))
        self.screen.blit(hab_surface, (20, y_offset))
        for i, habilidad in enumerate(entity.habilidades):
            estado = "✅" if habilidad.td_e_actual == 0 else f"🔄{habilidad.td_e_actual}"
            color = (200, 255, 200) if habilidad.puede_usar(entity) else (150, 150, 150)
            hab_info = f"  {i+1}. {habilidad.nombre} (PH: {habilidad.costo_ph}) {estado}"
            hab_surface = self.small_font.render(hab_info, True, color)
            self.screen.blit(hab_surface, (30, y_offset + 25 + i * 20))
    ...
    def set_ability_menu(self, entity):
        """Establece el menú de habilidades para una entidad"""
        menu_x = self.screen_width - 270
        menu_y = 100
        self.ability_menu = AbilityMenu(entity, (menu_x, menu_y))