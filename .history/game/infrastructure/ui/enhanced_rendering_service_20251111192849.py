...
class EnhancedRenderingService(RenderingService):
    ...
    def render_battle(self, battle, game_context: GameContext, valid_moves: List[Position] = None) -> None:
        ...
        for entity in battle._entities.values():
            ...
            # Resaltar Ricchard en (2,1) con borde verde fuerte y texto grande
            if getattr(entity, 'name', '') == 'Ricchard' and entity.position.x == 2 and entity.position.y == 1:
                pygame.draw.rect(
                    self.screen,
                    (0, 255, 0),
                    pygame.Rect(
                ...
                self.screen.blit(
                    text_surface,
                    (self.grid_offset[0] + entity.position.x * self.cell_size + 4,
                     self.grid_offset[1] + entity.position.y * self.cell_size + 4)
                )
    ...
    def _render_entity(self, entity, is_selected: bool = False, is_marked_for_dash: bool = False):
        ...
        pos = entity.position
        team = entity.team
        # Usar la instancia actual de MovementVisualizer para conversión alineada
        screen_xy = self.movement_visualizer._position_to_screen(pos, self.camera_offset)
        screen_x = screen_xy[0] + self.cell_size // 2
        screen_y = screen_xy[1] + self.cell_size // 2
        ...
        bar_y = screen_y - self.cell_size // 2 + 4
