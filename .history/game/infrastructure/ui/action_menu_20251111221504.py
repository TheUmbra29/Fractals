...
class ActionMenu:
    ...
    def _build_available_actions(self) -> List[dict]:
        """Construye lista de acciones con información de PH y cooldowns"""
        actions = [
            {
                "name": "Moverse", 
                "key": "M", 
        ...
        # Añadir habilidades tipo Habilidad
        for habilidad in self.entity.habilidades:
            actions.append({
                "name": habilidad.nombre,
                "key": habilidad.id[0].upper(),  # Primera letra del id como tecla rápida
                "type": habilidad.id,
                "enabled": self.entity.puede_usar_habilidad(habilidad.id),
                "description": habilidad.descripcion,
                "ph_cost": habilidad.costo_ph,
                "cooldown": habilidad.td_e_actual,
                "max_cooldown": habilidad.td_e_max
            })
        return actions
